...
function getBgColor(attempt, i) {
  ...
  let attempLetter = attempt[i];
  if (attempLetter === undefined || secret.indexOf(attempLetter) === -1) {
    return GREY;
  }
  if (correctLetter === attempLetter) {
    return GREEN;
  }
  return YELLOW;
}
...
function buildKeyboard() {
  biuldKeyboardRow("qwertyuiop", false);
  biuldKeyboardRow("asdfghjkl", false);
  biuldKeyboardRow("zxcvbnm", true);
}

function biuldKeyboardRow(letters, isLastRow) {
  let row = document.createElement("div");
  if (isLastRow) {
